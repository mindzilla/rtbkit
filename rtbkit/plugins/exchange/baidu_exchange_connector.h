...
struct BaiduExchangeConnector : public HttpExchangeConnector {
    BaiduExchangeConnector(ServiceBase & owner, const std::string & name) :
    ...
    BaiduExchangeConnector(const std::string & name, std::shared_ptr<ServiceProxies> proxies) :
    ...
    Datacratic::HttpResponse getResponse(HttpAuctionHandler const & handler,
        ...
        for (unsigned spotNum = 0; spotNum < current->responses.size(); ++spotNum) {
          result += std::to_string(current->responses.size());
